...
> **확인 방법**: 브라우저에서 http://localhost/DVWA 접속 후 Setup 페이지에서 "Create / Reset Database" 클릭
...
### 시나리오: SQL Injection 공격
**목표**: 로그인 우회를 통해 인증 없이 접근
...
> **예상 결과**: SQL 쿼리 우회를 통해 인증 없이 데이터 접근 성공
...
| 증상       | 원인        | 해결 방법                 |
| -------- | --------- | --------------------- |
| DB 연결 실패 | MySQL 미실행 | `service mysql start` |
| 페이지 흰 화면 | PHP 오류    | `php.ini` 설정 확인       |
| 권한 오류    | 파일 권한 부족  | chmod/chown 재설정       |
| 로그인 안됨   | DB 초기화 안됨 | Setup 페이지에서 DB 생성     |
